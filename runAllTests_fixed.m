...
for k = 1:numel(roadCases)
    ...
    % Select which road case to use - via block parameters, not variables
    in = in.setBlockParameter(manualSwitchPath, "CurrentSetting", ...
        num2str(roadCases(k).SwitchValue));

    if roadCases(k).Scenario ~= ""
        in = in.setBlockParameter(signalEditorPath, "ActiveScenario", ...
            roadCases(k).Scenario);
    end
